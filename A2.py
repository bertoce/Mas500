...
# file handler
fh = logging.FileHandler(r'/Users/Berto/Desktop/mas500/log.txt')
...
class mediacloudTest(unittest.TestCase):
    ...
    def testCount(self):
        ...
        logger.info('Querying for word [CLINTON] in September 2016...')
        res = mc.sentenceCount("clinton", solr_filter=[
            mc.publish_date_query(datetime.date(2016, 9, 1), datetime.date(2016, 9, 30)), 'tags_id_media:1'])
        count = res['count']

        logger.info('Finding results on Mediacloud...')
        print ("SENTENCES CONTAINING WORD 'CLINTON' DURING SEPTEMBER 2016: ", count)
        assert count > 0
